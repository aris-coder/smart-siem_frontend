...
import { ShieldAlert, AlertTriangle, Info, Shield, Users, Server, Globe } from 'lucide-react'
...
export function SeverityDonutChart({ critical, high, warning, info }: SeverityDonutChartProps) {
  const [hoveredIdx, setHoveredIdx] = useState<number | null>(null)
  
  const data = useMemo(() => [
    { label: 'Critique', value: critical || 0, color: '#ef4444', hoverColor: '#dc2626', shadow: 'rgba(239, 68, 68, 0.4)' },
    { label: 'Élevé', value: high || 0, color: '#f97316', hoverColor: '#ea580c', shadow: 'rgba(249, 115, 22, 0.4)' },
    { label: 'Avertissement', value: warning || 12, color: '#eab308', hoverColor: '#ca8a04', shadow: 'rgba(234, 179, 8, 0.4)' },
    { label: 'Information', value: info || 25, color: '#3b82f6', hoverColor: '#2563eb', shadow: 'rgba(59, 130, 246, 0.4)' },
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border border-[var(--line)] rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-[var(--sea-ink)]">Alertes par Sévérité</h3>
          <p className="text-[10px] text-[var(--sea-ink-soft)]">Distribution des alertes en cours</p>
        </div>
        <Shield className="size-4 text-[var(--sea-ink-soft)]" />
      </div>
      ...
      <div className="flex flex-1 flex-col sm:flex-row items-center justify-center gap-6 min-h-[180px]">
        ...
        <div className="relative size-36 shrink-0">
            ...
            {segments.map((seg, idx) => (
              <circle
                ...
                style={{
                  ...
                  filter: hoveredIdx === idx ? `drop-shadow(0 0 4px ${seg.color})` : 'none',
                }}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none">
            ...
            <span className="text-[10px] text-[var(--sea-ink-soft)] uppercase font-semibold tracking-wider">
          ...
          {segments.map((seg, idx) => (
            <div 
              ...
              className={`flex items-center justify-between px-2 py-1 rounded-lg border transition-all cursor-pointer ${
                hoveredIdx === idx 
                  ? 'border-[var(--sea-ink-soft)] bg-zinc-50 dark:bg-zinc-800/50 scale-[1.02]' 
                  : 'border-transparent'
              }`}
            >
              <div className="flex items-center gap-2">
                <span className="size-2.5 rounded-full shrink-0" style={{ backgroundColor: seg.color }} />
                <span className="text-xs font-medium text-[var(--sea-ink)]">{seg.label}</span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <span className="font-semibold text-[var(--sea-ink)]">{seg.value}</span>
                <span className="text-[10px] text-[var(--sea-ink-soft)] font-medium">({seg.percent.toFixed(0)}%)</span>
              </div>
            </div>
          ))}
        </div>
...
export function EventEvolutionChart() {
  ...
  const dataset = useMemo(() => [
    { time: '08:00', count: 180 }, { time: '09:00', count: 320 }, { time: '10:00', count: 450 },
    { time: '11:00', count: 390 }, { time: '12:00', count: 280 }, { time: '13:00', count: 310 },
    { time: '14:00', count: 520 }, { time: '15:00', count: 680 }, { time: '16:00', count: 710 },
    { time: '17:00', count: 490 }, { time: '18:00', count: 340 }, { time: '19:00', count: 290 },
    { time: '20:00', count: 210 }, { time: '21:00', count: 190 }, { time: '22:00', count: 250 },
    { time: '23:00', count: 540 }, { time: '00:00', count: 610 }, { time: '01:00', count: 820 }, // SSH attack simulation
    { time: '02:00', count: 850 }, { time: '03:00', count: 480 }, { time: '04:00', count: 230 },
    { time: '05:00', count: 150 }, { time: '06:00', count: 140 }, { time: '07:00', count: 160 }
  ], [])

  const maxVal = useMemo(() => Math.max(...dataset.map(d => d.count)), [dataset])
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border border-[var(--line)] rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-[var(--sea-ink)]">Évolution des Événements (24h)</h3>
          <p className="text-[10px] text-[var(--sea-ink-soft)]">Volume de logs analysés par heure</p>
        </div>
      ...
      <div className="relative flex-1 w-full mt-2 select-none">
        ...
        {hoveredIdx !== null && points[hoveredIdx] && (
          ...
          >
            <div className="font-semibold text-[var(--sea-ink)]">Hour: {points[hoveredIdx].time}</div>
            <div className="text-blue-500 font-mono font-bold mt-0.5">Events: {points[hoveredIdx].count}</div>
          </div>
        )}
      </div>

      <div className="flex justify-between mt-2.5 text-[9px] font-mono text-[var(--sea-ink-soft)] px-5">
...
export function TopSourcesChart({ ips = [] }: TopSourcesChartProps) {
  ...
  const data = useMemo(() => {
    const list = ips && ips.length > 0 ? ips : ['192.168.4.120', '10.20.101.45', '172.16.88.2', '185.190.140.23', '192.168.1.1']
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border border-[var(--line)] rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-[var(--sea-ink)]">Top Sources IP</h3>
          <p className="text-[10px] text-[var(--sea-ink-soft)]">Hôtes les plus actifs en volume d'événements</p>
        </div>
        ...
        {data.map((item, idx) => (
          <div key={item.ip} className="space-y-1.5">
            <div className="flex justify-between items-center text-xs font-mono">
              <span className="font-medium text-[var(--sea-ink)] hover:text-blue-500 transition-colors cursor-pointer">
                {item.ip}
              </span>
              <span className="font-bold text-[var(--sea-ink)]">{item.count} evts</span>
            </div>
...
export function ThreatTypesChart() {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border border-[var(--line)] rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-[var(--sea-ink)]">Types de Menaces</h3>
          <p className="text-[10px] text-[var(--sea-ink-soft)]">Distribution des typologies d'attaques</p>
        </div>
        <ShieldAlert className="size-4 text-[var(--sea-ink-soft)]" />
      </div>
      ...
      <div className="flex items-end justify-between gap-3 h-[180px] pt-4 px-2 select-none relative">
            ...
            <div className="w-full bg-zinc-100 dark:bg-zinc-800 rounded-t-lg h-[130px] flex items-end overflow-hidden relative">
              <div 
                ...
                style={{ 
                  ...
                  boxShadow: hoveredIdx === idx ? `0 0 12px ${t.color}` : 'none'
                }}
              />
            </div>
            {/* Label */}
            <span className="text-[9px] text-[var(--sea-ink-soft)] font-medium text-center truncate max-w-full">
              {t.type.split(' ')[0]}
            </span>
          </div>
        ))}
        ...
        {hoveredIdx !== null && threats[hoveredIdx] && (
          <div className="absolute top-2 left-1/2 -translate-x-1/2 z-10 p-2 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-md text-xs">
            <p className="font-semibold text-[var(--sea-ink)]">{threats[hoveredIdx].type}</p>
            <p className="font-mono mt-0.5 text-zinc-500">
              <span style={{ color: threats[hoveredIdx].color }}>{threats[hoveredIdx].count} evts</span> ({threats[hoveredIdx].value}%)
            </p>
          </div>
        )}
      </div>
...
export function LoginFailuresChart() {
  ...
  const timeline = useMemo(() => [
    ...
    { time: '12h-14h', count: 154, threat: 'Élevé (Attaque force brute IP externe)' },
    { time: '14h-16h', count: 62, threat: 'Moyen' },
    { time: '16h-18h', count: 28, threat: 'Faible' }
  ], [])

  const maxFailures = useMemo(() => Math.max(...timeline.map(t => t.count)), [timeline])

  return (
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border border-[var(--line)] rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-[var(--sea-ink)]">Échecs de Connexion (16h)</h3>
          <p className="text-[10px] text-[var(--sea-ink-soft)]">Tendance des connexions rejetées</p>
        </div>
      ...
      <div className="flex items-end justify-between gap-2.5 h-[130px] select-none relative pt-4">
        {timeline.map((item, idx) => {
          ...
          return (
            ...
            >
              ...
              {/* Label */}
              <span className="text-[8px] font-mono text-[var(--sea-ink-soft)]">
                {item.time}
              </span>
            </div>
          )
        ...
        {hoveredIdx !== null && timeline[hoveredIdx] && (
          <div className="absolute top-2 left-1/2 -translate-x-1/2 z-10 p-2.5 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-md text-xs w-[180px]">
            <p className="font-semibold text-[var(--sea-ink)]">Période : {timeline[hoveredIdx].time}</p>
            <p className="mt-1 font-bold text-red-500">{timeline[hoveredIdx].count} Échecs</p>
            <p className="mt-0.5 text-[10px] text-zinc-400">Statut : {timeline[hoveredIdx].threat}</p>
          </div>
        )}
      </div>
    </div>
...
export function SuspiciousActivityHeatmap() {
  const [hoveredCell, setHoveredCell] = useState<{ day: string, hour: string, score: number, desc: string } | null>(null)

  const days = ['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim']
  const hourBlocks = ['00h-04h', '04h-08h', '08h-12h', '12h-16h', '16h-20h', '20h-00h']
  ...
  const cellDetails = (dayIdx: number, hourIdx: number) => {
    ...
    if (val === 3) label = 'Alerte Critique (Accès administrateur hors-horaires / Exfiltration)'

    return { day, hour, score: val, desc: label }
  }

  const getCellBg = (val: number) => {
    if (val === 3) return 'bg-red-500 dark:bg-red-500 shadow-[0_0_6px_rgba(239,68,68,0.5)] border border-red-400'
    if (val === 2) return 'bg-orange-500 dark:bg-orange-500 border border-orange-400'
    if (val === 1) return 'bg-yellow-500 dark:bg-yellow-500 border border-yellow-400'
    return 'bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-800'
  }

  return (
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border border-[var(--line)] rounded-xl p-5 shadow-sm relative">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-[var(--sea-ink)]">Activité Suspecte (Heatmap)</h3>
          <p className="text-[10px] text-[var(--sea-ink-soft)]">Recherche d'anomalies de trafic par plage horaire</p>
        </div>
        <Server className="size-4 text-[var(--sea-ink-soft)]" />
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center gap-2 mt-2 select-none">
        ...
        <div className="flex items-center pl-8 mb-1">
          {hourBlocks.map((block) => (
            <span key={block} className="flex-1 text-[8px] font-mono text-[var(--sea-ink-soft)] text-center">
              {block}
            </span>
          ))}
        </div>
        ...
        <div className="space-y-1.5">
          {days.map((day, dayIdx) => (
            <div key={day} className="flex items-center gap-2">
              {/* Day label */}
              <span className="w-6 text-[9px] font-semibold text-[var(--sea-ink-soft)]">
                {day}
              </span>
                ...
                {hourBlocks.map((hour, hourIdx) => {
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={() => setHoveredCell(cellDetails(dayIdx, hourIdx))}
        ...
        <div className="flex items-center gap-4 mt-4 pt-3 border-t border-[var(--line)] text-[8px] text-[var(--sea-ink-soft)] justify-center">
      ...
      {hoveredCell && (
        <div className="absolute z-20 top-2 left-1/2 -translate-x-1/2 p-2 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-md text-xs w-[220px]">
          <div className="flex justify-between items-center font-bold text-[var(--sea-ink)] mb-0.5">
            <span>{hoveredCell.day} - {hoveredCell.hour}</span>
            <span className={`px-1.5 py-0.5 rounded text-[8px] font-semibold ${
              ...
              {hoveredCell.score === 3 ? 'Critique' : hoveredCell.score === 2 ? 'Moyen' : hoveredCell.score === 1 ? 'Mineur' : 'Normal'}
            </span>
          </div>
          <p className="text-[10px] text-zinc-500 dark:text-zinc-400 leading-snug">{hoveredCell.desc}</p>
        </div>
      )}
    </div>
  )
